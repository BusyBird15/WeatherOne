    ...
    public int matchN(Object[] args, CallContext ctx) {
        ...
        while (decl != null) {
            if (i2 < min_args) {
                ...
                key_i = key_i2 + 1;
                Object value3 = Keyword.searchForKeyword(args, min_args + opt_args, this.lambda.keywords[key_i2]);
                if (value3 == Special.dfault) {
                    value3 = this.lambda.evalDefaultArg(opt_i, ctx);
                }
                opt_i++;
                i = i2;
                ...
                if (elementType == Type.objectType) {
                    ...
                    value2 = Array.newInstance(elementType.getReflectClass(), rem);
                    int j = 0;
                    while (j < rem) {
                        try {
                            Array.set(value2, j, elementType.coerceFromObject(args[i2 + j]));
                            j++;
