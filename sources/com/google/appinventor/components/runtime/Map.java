...
public class Map extends MapFeatureContainerBase implements MapFactory.MapEventListener {
    ...
    public void Save(String path) {
        ...
        if (path.startsWith("/") || path.startsWith("file:/")) {
            final File target = path.startsWith("file:") ? new File(URI.create(path)) : new File(path);
        ...
        new FileWriteOperation($form(), this, "Save", path, $form().DefaultFileScope(), false, true) {
            ...
            public void processFile(ScopedFile file) {
                Map.this.doSave(featuresToSave, new File(URI.create(FileUtil.resolveFileName(this.form, file))));
            }
        }.run();
    }
